...
/*	3)	Add "Boilerplate Admin" Page options */
	...
	if ( ! function_exists( 'jquery_js_setting' ) ):
		function jquery_js_setting() {
			...
			$version = (isset($options['jquery_version']) && $options['jquery_version'] && $options['jquery_version'] !== '') ? $options['jquery_version'] : '1.8.2';
...
/*	4)	Create functions to add above elements to pages */
	...
	if ( ! function_exists( 'add_jquery_script' ) ):
		function add_jquery_script() {
			$cache = cache_buster();
			$options = get_option('plugin_options');
			$version = ($options['jquery_version']) ? $options['jquery_version'] : '1.8.2';
